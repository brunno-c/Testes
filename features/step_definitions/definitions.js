...
geraEmailValido = () => `testeandroid@${(Math.random() * (maxRandomValue - 0).toFixed(0))}.com`;
...
Then("Test DONE -> Written {string} on the page", async function (arg1) {
    await driver.pause(maxRandomValue);
    // await driver.wait(until.driver.getText(`android=new UiSelector().textMatches("${arg1}")`), very_long_time);
...
When("I move the page", async function () {
    await driver.pause(long_time);
...
Given("I click on the button with description{string} ,classname {string} and index = {string}", async function (arg1, arg2, arg3) {
    await driver.pause(short_time);
    await driver.click(`android=new UiSelector().description("${arg1}")className("${arg2}").enabled(true).instance(${arg3})`);
...
Then("Test finished: The {string} icon is on the screen", async function (arg1) {
...
Given("The field with classname {string} and index {string} is filled with {string}", async function (arg1, arg2, arg3) {

    await driver.pause(long_time);
    await driver.setValue(`android=new UiSelector().className("${arg1}").enabled(true).instance(${arg2})`, arg3);
